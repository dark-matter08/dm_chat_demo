...
from kivy.uix.screenmanager import Screen, ScreenManager, Screen
...
class ConnectPage(GridLayout):
    ...
    def join_button(self, instance):
        ...
        print(f"Attempting to join {port}:{ip} as {username}")

        with open("prev_details.txt", "w") as f:
            f.write(f"{port},{ip},{username}")

        info = f"Attempting to join {port}:{ip} as {username}"
    ...
    def connect(self, _):
        ...
        if not client_socket.connect(ip, port, username, show_error):
            return
